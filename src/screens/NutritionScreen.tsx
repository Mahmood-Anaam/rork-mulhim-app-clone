...
import { MealStructure, MealSuggestion, WeeklyMealPlan, DailyMealPlan, GroceryItem, GroceryList } from "@/types/fitness";
...
export const NutritionScreen = () => {
  ...
  const targetCalories = Math.round(getTargetCalories ? getTargetCalories() : 2000);
  const latestWeight = getCurrentWeight ? getCurrentWeight() : 70;
  ...
  const handleGenerateCustomMeal = async () => {
    ...
    try {
      ...
      const result = await openAIService.generateStructuredData<any>(
        `قم بتحليل هذه الوجبة وإعطائي بياناتها الغذائية: "${customMealInput}"
